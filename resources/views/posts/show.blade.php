    ...
    <div class="offset-1 col-9">
        ...
        @endif
            <div class="card-body word-wrap p-4">
              ...
              <p class="card-text">datum: {{date("d/m/Y", strtotime($post->created_at))}} <br> Auteur: {{$post->users->name}} <br> @if(!empty($post->category_id)) Categorie: {{$post->categories->name}}</p> @else @endif <br>
